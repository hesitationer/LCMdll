...
void subDustBorder(Mat &imgIn, Mat &imgOut , vector<vector<Point>> &contours ,int cameraFlag, int imgFlag)
{
	...
	imgIn.convertTo(gradientTftThreshold_point, CV_16S);
...
void subDustBorder_mura(Mat &imgIn, Mat &imgOut, vector<vector<Point>>&contours ,int cameraFlag, int imgFlag)
{
	...
	//cv::threshold(imgIn, gradientTftThreshold_point, (*canshuX)._TFTHOLD, 255, THRESH_BINARY);
	cv::threshold(imgIn, gradientTftThreshold_point, (*canshuX)._TFTHOLD_muras[cameraFlag][imgFlag], 255, THRESH_BINARY);
...
void subDustBorder_subTmp(Mat &imgIn, Mat &imgOut, vector<vector<Point>> &contours, int imgFlag)
{
	contours.clear();
	Mat gradientTftThreshold_point, gradientResult_point, gradientResultThreshold_point;
	...
	if (imgFlag == 0)
	{
		Sub_Threshold = (*canshuX)._subThresh[0][0];
	}
	else 
	{
		Sub_Threshold = (*canshuX)._subThresh[0][imgFlag];
	}
	cv::threshold(imgIn, gradientTftThreshold_point, Sub_Threshold, 255, THRESH_BINARY);  
