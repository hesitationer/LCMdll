...
void setmethodTrackbar(Mat imgtest, int camera_flag, int imgFlag)
{
	...
	createTrackbar("numPoint", "method", &numPoint, 700, onMethod, &sendImg1);
	...
	createTrackbar("方法4", "method", &subBGRThresh, 180, onMethod, &sendImg1);
	...
	detect(imgtest, camera_flag, imgFlag, false);
...
void setsecondParameterTrackbar_before(Mat imgtest, int camera_flag, int imgFlag, int method) {
...
void setsecondParameterTrackbar(Mat imgtest, int camera_flag, int imgFlag, int method) {
	...
	createTrackbar("启动调试", "secondParameter", &beginDug, 1, onSecondParameter, &sendImg1);
	...
	(*sendImg1).debug_imgFlag = imgFlag;
	(*sendImg1).debug_method = method;
	(*sendImg1).debug_cutImg = (*sendImg1).imgTftColorCuts[camera_flag][imgFlag].clone();
