...
void hough_detect(Mat& image, vector<Point2f>& res_points, vector<int>sort_dxy, int binary_thres, bool show_img)
{
	...
	vector<Vec4i>lines;
	//////////11.26改了霍夫变换的参数/////////
	HoughLinesP(edge, lines, 0.75, CV_PI / 180, 80, 100, 0);  //  0.68   120   100 00
...
void getRoi(int cameraFlag, bool isBuildtemplate)
{
	...
	default:
		...
		sort_dxy.push_back(20);
		hough_detect((*sendImg1).imgTftColorCut[cameraFlag], (*sendImg1).pts1[cameraFlag], sort_dxy, 100, false);
		break;
	case 0:
		sort_dxy.push_back(15);
		sort_dxy.push_back(12);
