...
{
	Mat tmp_gradientTFT = imgIn.clone();
...
void setMouseChoseArea(Mat &imgIn, Mat &imgOut)
{
	//鼠标圈定
	(*sendImg1).getMouseChooseimg = (*sendImg1).tftResultShow.clone();  //为鼠标圈定的图赋值
	waitKey(0);
	system("color 9F");
	Mat temp1;
	while (waitKey(30) != 27)
...
void initChangePartOfTheRegionalGradient(Mat ImgIn, Mat bordertft, Mat &maskChangeGradietRCorner, Mat &maskChangeGradientDrawIn, CanShuMask canshumask)
{
	...
	getRCornerMask(bordertft, maskChangeGradietRCorner, canshumask._keneralsize);
	//获取需要调整梯度的内缩区域模板 ， 需要调整的地方为255，无需调整的为0
	getDrawInAreaMask(ImgIn, maskChangeGradientDrawIn, canshumask._size_x_left, canshumask._size_x_right, canshumask._size_y_up, canshumask._size_y_down);
	//改变圆弧区域梯度值    A=  A*f1
}
...
void initChangePartOfTheRegionalGradient(Mat &maskChangeGradietRCorner, Mat &maskChangeGradientDrawIn, CanShuMask canshumask)
{
	...
	//对border进行膨胀，获取mask模板  需要调整的地方为255，无需调整的为0
	getRCornerMask((*sendImg1).BorderTFT, maskChangeGradietRCorner, canshumask._keneralsize);
	//获取需要调整梯度的内缩区域模板 ， 需要调整的地方为255，无需调整的为0
	getDrawInAreaMask((*sendImg1).GradientResult_point[0][0], maskChangeGradientDrawIn, canshumask._size_x_left, canshumask._size_x_right, canshumask._size_y_up, canshumask._size_y_down);
	//改变圆弧区域梯度值    A=  A*f1
}
