...
void hough_detect(Mat& image, vector<Point2f>& res_points, vector<int>sort_dxy, int binary_thres, bool show_img)
{
	...
	imwrite((*sendImg1).saveFileDir + "Roi_hough.png", img_draw);
...
//用来获取白屏IOU区域信息
void getRoi(int cameraFlag, bool isBuildtemplate)
{
	...
	//imwrite("D:\\project\\bluetmpimgTftColorCut.png", (*sendImg1).imgTftColorCut[cameraFlag]);
	...
	default:
		...
		hough_detect((*sendImg1).imgTftColorCut[cameraFlag], (*sendImg1).pts1[cameraFlag], sort_dxy, (*canshuX)._houghthr[cameraFlag][0], false);
		break;
	case 1:
		sort_dxy.push_back((*canshuX)._sort_dxy1[cameraFlag][0]);
		sort_dxy.push_back((*canshuX)._sort_dxy2[cameraFlag][0]);
	...
	//imwrite("D:\\project\\bluetmpimgTftColorCut2.png", (*sendImg1).imgTftColorCut[cameraFlag]);
	...
	if (isBuildtemplate == true)
	{
		...
		if (cameraFlag == 0)
		{
			...
			ofstream OutFile1((*sendImg1).h_w_tmp_file);
			OutFile1 << (*sendImg1).PersPectiveTransformation_hight; //把字符串内容"This is a Test!"，写入Test.txt文件
			OutFile1 << "\n";
			OutFile1 << (*sendImg1).PersPectiveTransformation_weight; //把字符串内容"This is a Test!"，写入Test.txt文件
			OutFile1 << "\n";
			OutFile1.close();//关闭Test.txt文件
		}
		...
		//imwrite("D:\\project\\bluetmpimgTftColorCut3.png", (*sendImg1).imgTftColorCut);
		...
		if (cameraFlag == 0)
		{
			...
			ifstream InFile1((*sendImg1).h_w_tmp_file);;
			if (InFile1) // 有该文件  
			{
				//getline逐行读入
				while (getline(InFile1, line)) // line中不包括每行的换行符  
				{
					//cout << line << endl;
					if (f == 0)
					{
						(*sendImg1).PersPectiveTransformation_hight = atof(line.c_str());
						f++;
					}
					else
					{
						(*sendImg1).PersPectiveTransformation_weight = atof(line.c_str());
						f = 0;
						(*sendImg1).pts1_tmp.push_back(p);
					}
				}
		...
		//进行透视变换
		warpPerspective((*sendImg1).imgTftColorCut[cameraFlag], (*sendImg1).imgTftColorCut[cameraFlag], M, Size((*sendImg1).PersPectiveTransformation_weight, (*sendImg1).PersPectiveTransformation_hight));
		//(*sendImg1).imgTftColorCut[cameraFlag] = img.clone();
...
void getRCornorResult(int cameraFlag)
{
	...
	string path = (*sendImg1).Initpath + "//R_tft//";
	newmkdir(path);
	find_R_cornor(R_tft_cut_padding, pts, R_size, path, (*sendImg1).res);

	string path_diff = (*sendImg1).Initpath + "//R_dif//";
	newmkdir(path_diff);
